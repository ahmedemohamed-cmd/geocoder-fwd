        ...
        for k in keys:
            if k["status"] == "active" and k.get("key_enc"):
                await apisix_admin.upsert_consumer(
                    k["id"],
                    api_key=security.decrypt_key(k["key_enc"]),
                    in_group=in_group,
                    tenant_id=tenant_id,
                )
            else:
...
from .auth import current_identity, get_pool, get_redis, require_admin, require_tenant
from .models import (
    AdminCreate,
    AdminOut,
    AdminUpdate,
...
def build_app(pool=None, redis=None) -> FastAPI:
    ...
    @app.post("/admin/admins", response_model=AdminOut, status_code=201, tags=[router_tag_admin])
    async def add_admin(
        body: AdminCreate, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
        ...
        try:
            ...
            raise HTTPException(
                status.HTTP_502_BAD_GATEWAY, f"admin created but IdP provisioning failed: {e}"
            ) from e
        return admin

    @app.patch("/admin/admins/{email}", response_model=AdminOut, tags=[router_tag_admin])
    async def modify_admin(
        email: str, body: AdminUpdate, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
        ...
        try:
            ...
            raise HTTPException(status.HTTP_502_BAD_GATEWAY, f"IdP update failed: {e}") from e
        return admin
    ...
    async def remove_admin(
        ...
        try:
            ...
            raise HTTPException(status.HTTP_502_BAD_GATEWAY, f"IdP delete failed: {e}") from e
        return Response(status_code=204)

    @app.post("/admin/admins/{email}/reset-password", status_code=204, tags=[router_tag_admin])
    ...
    # ── admin: tenants CRUD ──────────────────────────────────────────────────
    @app.post("/admin/tenants", response_model=TenantOut, status_code=201, tags=[router_tag_admin])
    async def create_tenant(
        body: TenantCreate, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
        ...
        try:
            ...
            raise HTTPException(
                status.HTTP_502_BAD_GATEWAY, f"tenant created but IdP provisioning failed: {e}"
            ) from e
    ...
    async def list_tenants(
        include_deleted: bool = False, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
    ...
    async def get_tenant(
        tenant_id: str, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
    ...
    async def add_tenant_user(
        ...
        try:
            ...
            raise HTTPException(
                status.HTTP_502_BAD_GATEWAY, f"user created but IdP provisioning failed: {e}"
            ) from e
        return user

    @app.patch(
        "/admin/tenants/{tenant_id}/users/{email}",
        response_model=TenantUserOut,
        tags=[router_tag_admin],
    )
    async def modify_tenant_user(
        tenant_id: str,
        email: str,
        ...
        try:
            user = await repo.set_user_status(
                pool, tenant_id=tenant_id, email=email, status=body.status
            )
        except repo.NotFound as e:
            raise _not_found(e) from e
        try:
    ...
    @app.delete(
        "/admin/tenants/{tenant_id}/users/{email}", status_code=204, tags=[router_tag_admin]
    )
    async def delete_tenant_user(
        tenant_id: str, email: str, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
    ...
    @app.post("/admin/tenants/{tenant_id}/reset-password", status_code=204, tags=[router_tag_admin])
    ...
    async def create_plan(
        body: PlanCreate, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
    ...
    async def get_plan(plan_id: str, _: Identity = Depends(require_admin), pool=Depends(get_pool)):
        try:
    ...
    async def update_plan(
        plan_id: str, body: PlanUpdate, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
    ...
    async def delete_plan(
        plan_id: str, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
    ...
    @app.get(
        "/admin/tenants/{tenant_id}/invoices",
        response_model=list[InvoiceOut],
        tags=[router_tag_admin],
    )
    async def admin_tenant_invoices(
        tenant_id: str, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        return await repo.list_invoices(pool, tenant_id=tenant_id)

    @app.post(
        "/admin/invoices/{invoice_id}/pay", response_model=InvoiceOut, tags=[router_tag_admin]
    )
    async def mark_paid(
        invoice_id: str, _: Identity = Depends(require_admin), pool=Depends(get_pool)
    ):
        try:
    ...
    async def get_key(
        key_id: str, ident: Identity = Depends(require_tenant), pool=Depends(get_pool)
    ):
        try:
    ...
    async def usage_history(
        ...
        rows = await repo.usage_history(
            pool, ident.tenant_id, period_from=period_from, period_to=period_to
        )
        return [
            UsageHistoryRow(
    ...
    async def my_invoice(
        invoice_id: str, ident: Identity = Depends(require_tenant), pool=Depends(get_pool)
    ):
        try:
    ...
    async def usage_sink(request: Request, pool=Depends(get_pool), redis=Depends(get_redis)):
        ...
        if (
            config.USAGE_SINK_SECRET
            and request.query_params.get("token") != config.USAGE_SINK_SECRET
        ):
...
def _key_public(rec: dict) -> dict:
    return {
        k: rec[k]
        for k in ("id", "tenant_id", "name", "key_prefix", "scopes", "status", "created_at")
    }
