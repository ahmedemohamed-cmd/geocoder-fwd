...
async def test_admin_crud_cycle(cp_client):
    ...
    r = await cp_client.post("/admin/admins", headers=h,
                             json={"email": "a2@x.io", "password": "password123"})
    assert r.status_code == 201 and r.json()["status"] == "active"
    assert (await cp_client.post("/auth/login",
            json={"email": "a2@x.io", "password": "password123"})).status_code == 200

    # duplicate → 409
    assert (await cp_client.post("/admin/admins", headers=h,
            json={"email": "a2@x.io", "password": "password123"})).status_code == 409

    # disable → login blocked; enable → works
    assert (await cp_client.patch("/admin/admins/a2@x.io", headers=h,
            json={"status": "disabled"})).json()["status"] == "disabled"
    assert (await cp_client.post("/auth/login",
            json={"email": "a2@x.io", "password": "password123"})).status_code == 403
    await cp_client.patch("/admin/admins/a2@x.io", headers=h, json={"status": "active"})

    # reset password
    assert (await cp_client.post("/admin/admins/a2@x.io/reset-password", headers=h,
            json={"email": "a2@x.io", "new_password": "NewPass_99"})).status_code == 204
    assert (await cp_client.post("/auth/login",
            json={"email": "a2@x.io", "password": "NewPass_99"})).status_code == 200

    # delete → gone
    assert (await cp_client.delete("/admin/admins/a2@x.io", headers=h)).status_code == 204
    assert (await cp_client.post("/auth/login",
            json={"email": "a2@x.io", "password": "NewPass_99"})).status_code == 401
...
async def test_cannot_remove_or_disable_last_admin(cp_client):
    ...
    assert (await cp_client.patch("/admin/admins/admin@example.com", headers=h,
...
async def test_tenant_user_cannot_manage_admins(cp_client):
    ...
    assert (await cp_client.post("/admin/admins", headers=h,
            json={"email": "x@x.io", "password": "password123"})).status_code == 403
