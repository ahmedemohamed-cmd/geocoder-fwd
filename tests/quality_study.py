...
from shared.address import is_address_query, normalize_address_text, parse_address_query
...
def _should(q, parsed, addr_detected, lean):
    ...
    if addr_detected:
        ...
        if parsed.get("city"):
            cv = parsed["city"]
            should += [
                {"match": {"addr_city": {"query": cv, "boost": 3}}},
        ...
        if parsed.get("suburb"):
            sv = parsed["suburb"]
            should += [
                {"match": {"addr_suburb": {"query": sv, "boost": 2}}},
...
def eval_named(case, results):
    osm_rank = next(
        (i + 1 for i, r in enumerate(results) if r.get("osm_id") == case["osm_id"]), None
    )
...
def eval_address(case, results):
    ...
    st_rank = next(
        (
            i + 1
            for i, r in enumerate(results)
            if rr.street_match(case["addr_street"], r.get("addr_street", ""))
        ),
        None,
    )
...
def main():
    ...
    print(
        f"# quality study: {len(named)} named + {len(addr)} address  (geo-bias Cairo, limit {LIMIT})\n"
    )
    ...
    for variant in VARIANTS:
        ...
        def work(case):
            ...
            if case["kind"] == "named":
                ...
                res["n_strict"].append(sr)
                res["n_lenient"].append(lr)
            else:
                er, srk = eval_address(case, hits)
                res["a_exact"].append(er)
                res["a_street"].append(srk)
        ...
        print(f"## {variant}")
        print(
            f"  named   strict @1/5/10 = {at(res['n_strict'], 1):.1f} / {at(res['n_strict'], 5):.1f} / {at(res['n_strict'], 10):.1f}"
            f"   lenient @1 = {at(res['n_lenient'], 1):.1f}"
        )
        print(
            f"  address exact  @1/5/10 = {at(res['a_exact'], 1):.1f} / {at(res['a_exact'], 5):.1f} / {at(res['a_exact'], 10):.1f}"
            f"   street @1/5 = {at(res['a_street'], 1):.1f} / {at(res['a_street'], 5):.1f}"
        )
        print(
            f"  took p50/p90 = {statistics.median(tk):.0f} / {sorted(tk)[int(0.9 * len(tk))]:.0f} ms"
