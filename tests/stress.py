            ...
            "function_score": {
                "query": {
                    "bool": {
                        ...
                        "should": [
                            {"multi_match": {"query": q,
                                "fields": ["name^5", "name.autocomplete^2",
                ...
                "functions": [
                    {"weight": 1.0},
                    {"field_value_factor": {"field": "offline_rank",
                        "modifier": "log1p", "factor": 1, "missing": 0},
                     "weight": 1.5},
                    {"gauss": {"centroid": {"origin": {"lat": CAIRO[0], "lon": CAIRO[1]},
                        "scale": "10km", "offset": "1km", "decay": 0.5}}, "weight": 2},
                    {"field_value_factor": {"field": "popularity",
                        "modifier": "log1p", "factor": 1, "missing": 0}, "weight": 1},
                ],
...
def geocode_url(q):
    qs = urllib.parse.urlencode({"q": q, "lat": CAIRO[0], "lon": CAIRO[1],
                                 "limit": 10, "vector": "false", "ai": "false"})
    return f"{GEO}?{qs}"
...
def run_level(target, conc, duration):
    ...
    return {
        "conc": conc, "rps": round(done / elapsed, 1), "ok": done, "err": errors,
        "p50": round(statistics.median(lats), 1) if n else 0,
        "p90": round(pct(0.90), 1), "p99": round(pct(0.99), 1),
        "max": round(lats[-1], 1) if n else 0,
        "es_cpu": round(statistics.mean(cpu_samples)) if cpu_samples else None,
        "es_q": queue, "es_rej": (rej1 - rej0) if (rej1 is not None and rej0 is not None) else None,
    }
...
def main():
    ...
    hdr = ("conc", "rps", "ok", "err", "p50ms", "p90ms", "p99ms", "maxms", "es_cpu%", "es_q", "es_rej")
    print("| " + " | ".join(hdr) + " |")
    print("|" + "|".join(["---"] * len(hdr)) + "|")
    for c in LEVELS:
        r = run_level(target, c, DURATION)
        print("| {conc} | {rps} | {ok} | {err} | {p50} | {p90} | {p99} | {max} | {es_cpu} | {es_q} | {es_rej} |".format(**r))
        sys.stdout.flush()
        time.sleep(COOLDOWN)
