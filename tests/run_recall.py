...
def main():
    ...
    def work(i):
        c = cases[i]
        out[i] = eval_named(c) if c["kind"] == "named" else eval_address(c)
        if (i + 1) % 100 == 0:
    ...
    lines = []
    lines.append("# Cairo recall report (1000 popular places/addresses)\n")
    lines.append(f"Queries geo-biased to downtown Cairo {CAIRO}, limit={LIMIT}, vector=off.\n")
    ...
    for key, label in [("strict_rank", "strict (same osm_id)"), ("lenient_rank", "lenient (name or ≤150 m)")]:
        lines.append("| %s | %.1f%% | %.1f%% | %.1f%% |" % (
            label, 100 * at(named, key, 1) / nN, 100 * at(named, key, 5) / nN, 100 * at(named, key, 10) / nN))
    lines.append("")
    ...
    for key, label in [("exact_rank", "exact (same osm_id)"), ("street_rank", "correct street")]:
        lines.append("| %s | %.1f%% | %.1f%% | %.1f%% |" % (
            label, 100 * at(addr, key, 1) / nA, 100 * at(addr, key, 5) / nA, 100 * at(addr, key, 10) / nA))
