...
async def test_user_crud_and_disable(cp_client):
    tenant, _ = await make_tenant(cp_client, admin_email="owner@u.io", admin_password="password123")
    ...
    assert (
        await cp_client.post("/auth/login", json={"email": "u2@u.io", "password": "password123"})
    ).status_code == 200
    ...
    dis = await cp_client.patch(
        f"/admin/tenants/{tid}/users/u2@u.io", headers=h, json={"status": "disabled"}
    )
    ...
    await cp_client.patch(
        f"/admin/tenants/{tid}/users/u2@u.io", headers=h, json={"status": "active"}
    )
    assert (
        await cp_client.post("/auth/login", json={"email": "u2@u.io", "password": "password123"})
    ).status_code == 200
...
async def test_user_mgmt_requires_admin(cp_client):
    ...
    assert (
        await cp_client.patch(
            f"/admin/tenants/{tid}/users/owner5@u.io", headers=h, json={"status": "disabled"}
        )
    ).status_code == 403
    assert (
        await cp_client.delete(f"/admin/tenants/{tid}/users/owner5@u.io", headers=h)
    ).status_code == 403
...
async def test_add_user_unknown_tenant_404(cp_client):
    atok = await admin_token(cp_client)
    r = await cp_client.post(
        "/admin/tenants/00000000-0000-0000-0000-000000000000/users",
        headers=bearer(atok),
        json={"email": "x@u.io", "password": "password123"},
    )
