...
async def test_enable_disable_key(cp_client):
    ...
    dis = await cp_client.patch(f"/keys/{kid}", headers=bearer(ttok), json={"status": "disabled"})
    assert dis.status_code == 200 and dis.json()["status"] == "disabled"

    en = await cp_client.patch(f"/keys/{kid}", headers=bearer(ttok), json={"status": "active"})
...
async def test_soft_delete_keeps_row_in_db(cp_client, pool):
    ...
    row = await pool.fetchrow("SELECT status, deleted_at FROM api_keys WHERE id=$1", created["id"])
...
async def test_tenant_isolation(cp_client):
    ...
    assert (
        await cp_client.patch(
            f"/keys/{keyA['id']}", headers=bearer(tokB), json={"status": "disabled"}
        )
    ).status_code == 404
