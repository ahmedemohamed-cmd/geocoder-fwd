...
async def test_admin_lists_tenant_users(cp_client):
    ...
    assert any(u["email"] == "owner@reset.io" and u["role"] == "tenant_user" for u in r.json())


async def test_admin_resets_password(cp_client):
    tenant, _ = await make_tenant(
        cp_client, admin_email="owner2@reset.io", admin_password="oldpassword1"
    )
    atok = await admin_token(cp_client)
    # old password works
    assert (
        await cp_client.post(
            "/auth/login", json={"email": "owner2@reset.io", "password": "oldpassword1"}
        )
    ).status_code == 200

    r = await cp_client.post(
        f"/admin/tenants/{tenant['id']}/reset-password",
        headers=bearer(atok),
    ...
    assert (
        await cp_client.post(
            "/auth/login", json={"email": "owner2@reset.io", "password": "BrandNew_99"}
        )
    ).status_code == 200
    assert (
        await cp_client.post(
